...
# for each user, go though each product and use rand function for 1/6 chance of leaving a review
...
(1..154).each do |product_id|
    # if rand(1..6) < 7 then
    Review.create!(user_name: demo.name, user_id: demo.id, product_id: product_id, headline: Faker::GreekPhilosophers.quote, body: Faker::Lorem.paragraph(sentence_count: 10, supplemental: false, random_sentences_to_add: 4), stars: 3)
    # end
end